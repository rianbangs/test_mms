   ...
   .responsive-div_top{
    ...
    <div class="row">
        <!-- <h4>Sales Comparison per Group</h4> -->
        <h4 style="font-size: 25px;margin-top: -4px;margin-left: 27px;">Sales Comparison per Group</h4>
    ...
    <script>
        ...
        function load_records()
        {
            ...
            if(range == 'Monthly')
            {
                ...
                $.ajax({
                    type: 'post',
                    url: '<?php echo base_url(); ?>Sales_item_group_ctrl/view_yearly_monthly_report',
                    data: {
                        'range': range,
                        'store_no': store_no,
                        'year': year,
                ...
                $.ajax({
                    type:'post',
                    url:'<?php echo base_url(); ?>Sales_item_group_ctrl/view_yearly_report',
                    data:{
                        'range'       :range,
                        'store_no'    :store_no,
                        'year'        :year,
        ...
        { 
            ...
            if(range == 'Monthly'){
                ...
                // condition for all stores
                if (store_no == "Select_all_store") {
                    loader();
                    $.ajax({
                        type:'post',
                        url:'<?php echo base_url(); ?>Sales_item_group_ctrl/get_yearly_monthly_report',
                        data:{
                            'range'       :range,
                            'store_no'    :store_no,
                            'year'        :year,
                        ...
                        success: function(response)
                        {
                            ...
                            link.download = 'Monthly '+report_type+' Report per Group.xls'; 
                        ...
                        function protectExcelFile(fileUrl) {
                            ...
                            xhr.onload = function() {
                                if (xhr.status === 200) {
                                    ...
                                    link.href = url;
                                    report_type = report_type.charAt(0).toUpperCase() + report_type.slice(1).toLowerCase();
                                    link.download = 'Monthly '+report_type+' Report per Group.xlsx';
                                    link.click();
                                }
                            };
                            xhr.send();
            ...
            // condition for yearly range
                ...
                if(store_no == 'Select_all_store'){
                    ...
                    success: function(response)
                    {
                        ...
                        report_type = report_type.charAt(0).toUpperCase() + report_type.slice(1).toLowerCase();
                        link.download = 'Yearly '+report_type+' Report per Group.xls';                        
                        document.body.appendChild(link);                                       
                        link.click();                                                          
                        document.body.removeChild(link);
                            ...
                            function protectExcelFile(fileUrl) {
                                ...
                                xhr.onload = function() {
                                    if (xhr.status === 200) {
                                        ...
                                        link.href = url;
                                        report_type = report_type.charAt(0).toUpperCase() + report_type.slice(1).toLowerCase();
                                        link.download = 'Yearly '+report_type+' Report per Group.xlsx';
                                        link.click();
                                    }
                                };
                                xhr.send();
